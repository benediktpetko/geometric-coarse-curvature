...
def find_midpoint(source, endpoint, distances):
...
def johnson_algorithm(weight_matrix):
    num_nodes = len(weight_matrix)
    ...
    for u in range(num_nodes):
        ...
        # Undo the reweighting using the potentials to get the actual distances
        shortest_distances[u] = shortest_paths - (potentials[u] - potentials[:-1])

    return shortest_distances

# def johnson_algorithm_with_midpoints(weight_matrix):
#     num_nodes = len(weight_matrix)
#     dummy_node = num_nodes
#
#     # Add a dummy row and column with zeros to the weight matrix
...
#     reweighted_weight_matrix = weight_matrix + (potentials[:-1] - potentials[-1])
#
#     # Run Dijkstra's algorithm from each node to compute shortest paths and midpoints
#     shortest_distances = np.zeros((num_nodes, num_nodes))
#     midpoints = np.full((num_nodes, num_nodes), -1)
#
#     for u in range(num_nodes):
#         shortest_paths, midpoints_row = dijkstra(reweighted_weight_matrix, u)
#
#         # Undo the reweighting using the potentials to get the actual distances
#         shortest_distances[u] = shortest_paths - (potentials[u] - potentials[:-1])
#
#         # Store the midpoints for each shortest path in the corresponding row of the midpoints array
#         midpoints[u] = midpoints_row
#     print(midpoints)
#
#     return shortest_distances, midpoints
...
#             if weight_matrix[u][v] != np.inf and distance[u] + weight_matrix[u][v] < distance[v]:
#                 distance[v] = distance[u] + weight_matrix[u][v]
#                 midpoints[v] = u  # Record the midpoint for node v
#                 heapq.heappush(priority_queue, (distance[v], v))
#
